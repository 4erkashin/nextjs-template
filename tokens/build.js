...
function varsFile(light, colorKeys) {
  ...
  if (!light.get("grid.module")) {
    throw new Error(
      "tokens/tokens.json primitive set must define grid.module",
    );
  }
...
/**
 * Product names for layout breakpoints, in kit order. Not a sort of
 * the JSON keys.
 */
const layoutBreakpointNames = ["mobile", "tablet", "desktop"];

/**
 * Turn one layout recipe into an `@media` string. Product code never
 * sees these features; it only picks the name.
 *
 * @param {string} name
 ...
 * @returns {string}
 */
function mediaQueryFromRecipe(name, recipe, tokens) {
  /** @type {string[]} */
  const features = [];

  const orientation = recipe.orientation;
  if (
    orientation &&
    ...
    if (value !== "portrait" && value !== "landscape") {
      throw new Error(
        `layout.${name}.orientation must be portrait or landscape`,
      );
    }
    features.push(`(orientation: ${value})`);
  }

  if (recipe.minWidth) {
    const width = tokens.get(`layout.${name}.minWidth`);
    if (!width) {
      throw new Error(`layout.${name}.minWidth did not resolve`);
    }
    features.push(`(min-width: ${width})`);
  }

  if (recipe.belowMinWidth) {
    const width = tokens.get(`layout.${name}.belowMinWidth`);
    if (!width) {
      throw new Error(`layout.${name}.belowMinWidth did not resolve`);
    }
    features.push(`(not (min-width: ${width}))`);
  }

  if (features.length === 0) {
    throw new Error(`layout.${name} must declare a query recipe`);
  }

  return `@media ${features.join(" and ")}`;
}
...
function queriesFile(tokens) {
  const group = tokenSets.primitive.layout;
  ...
  const extra = Object.keys(group).filter(
    (name) => !name.startsWith("$") && !layoutBreakpointNames.includes(name),
  );
  if (extra.length > 0) {
    throw new Error(
      `layout has unknown breakpoints ${extra.join(", ")}; expected ${layoutBreakpointNames.join(", ")}`,
    );
  }

  const mediaLines = layoutBreakpointNames.map((name) => {
    const recipe = group[name];
    if (!recipe || typeof recipe !== "object" || Array.isArray(recipe)) {
      throw new Error(`layout.${name} must be a query recipe group`);
    }
    const query = mediaQueryFromRecipe(name, recipe, tokens);
    return `  ${name}: ${jsString(query)},`;
  });
